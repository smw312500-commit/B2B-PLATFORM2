...
const PROD_LOG_KEY = 'label_prod_log_v1'
...
function getProdLog() {
  try {
    return JSON.parse(localStorage.getItem(PROD_LOG_KEY) || '{}')
  } catch { return {} }
}
...
export default function CompleteTab({ searched }) {
  ...
  const fetchReleases = async () => {
    ...
    setMachineLog(getProdLog())
  }

  useEffect(() => { fetchReleases() }, [])

  const handleComplete = async (id, labelCode) => {
    setCompleting(id)
    try {
      const log = getProdLog()
      const times = log[labelCode] || {}
  ...
  const completed = releases
    ...
    .filter((r) => inRange(r.release_date, searched))
  ...
  return (
    <div className="space-y-5">
      ...
      <section>
        ...
        <p className="text-xs text-gray-400 mb-3">완료 버튼 클릭 시: 재고 자동 차감 → 플랫폼으로 출고 신호 전송</p>

        {pending.length === 0 ? (
          <div className="text-sm text-gray-400 py-6 text-center border rounded bg-gray-50">
            완료 처리할 생산 주문 없음
          </div>
        ) : (
            ...
            {pending.map((r) => {
              ...
              return (
                <div key={r.id} className={`flex items-center justify-between border rounded px-4 py-3 ${
                  ...
                  <div>
                    <span className="font-mono font-bold text-sm">{r.label_code}</span>
                    <span className="text-gray-500 text-xs ml-3">{r.release_qty.toLocaleString()}장</span>
                  ...
                  <button onClick={() => handleComplete(r.id, r.label_code)} disabled={completing === r.id}
              ...
              {completed.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 border text-xs text-gray-500">
                    {r.started_at ? fmtDT(r.started_at) : '-'}
                  </td>
