...
const MATERIAL_INFO = {
  라벨원단: { unit: 'm',  hint: '1m → 라벨 25장' },
  잉크:     { unit: '통', hint: '1통 → 라벨 10,000장' },
}

const VALID_MATERIALS = ['라벨원단', '잉크']

// 엑셀 날짜 → YYYY-MM-DD 변환 (직렬번호 / Date객체 / 문자열 모두 처리)
function excelDateToISO(val) {
  if (!val && val !== 0) return null

  // xlsx cellDates:true 시 Date 객체로 옴
...
function downloadTemplate() {
  const ws = XLSX.utils.aoa_to_sheet([
    ...
    ['라벨원단', 100, '원단공급사A', '2026-05-22', '2026-06-01', ''],
    ['잉크',     5,   '잉크공급사B', '2026-05-22', '2026-06-01', '샘플'],
  ])
  ws['!cols'] = [{ wch: 12 }, { wch: 10 }, { wch: 16 }, { wch: 14 }, { wch: 14 }, { wch: 16 }]
...
function validateRow(row, idx) {
  ...
  if (!VALID_MATERIALS.includes(row.material_name))
    errors.push(`${idx + 1}행: 품목은 '라벨원단' 또는 '잉크'만 가능`)
  if (!row.order_qty || isNaN(Number(row.order_qty)) || Number(row.order_qty) <= 0)
    errors.push(`${idx + 1}행: 발주량이 올바르지 않습니다`)
  if (!row.order_date)
    errors.push(`${idx + 1}행: 발주일 형식 오류 (YYYY-MM-DD)`)
  if (!row.due_date)
    errors.push(`${idx + 1}행: 납기요청일 형식 오류 (YYYY-MM-DD)`)
  return errors
}
...
export default function OrderTab({ searched }) {
  ...
  const [form, setForm] = useState({
    material_name: '라벨원단', order_qty: '', supplier: '',
  ...
  const handleUpload = async () => {
    ...
    for (const [i, row] of previewRows.entries()) {
      try {
        ...
        failDetails.push(`${i + 1}행 (${row.material_name}): ${msg}`)
      }
    }
    setUploading(false)
  ...
  const filtered = searched
    ? orders.filter((o) => inRange(o.order_date, searched))
    : orders
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select value={form.material_name} onChange={(e) => setForm({ ...form, material_name: e.target.value })}
                ...
                <option>라벨원단</option><option>잉크</option>
              </select>
            </div>
            <div>
              <label className="text-xs text-gray-500">
                발주량
                <span className="ml-1 text-blue-500 font-medium">({MATERIAL_INFO[form.material_name]?.unit})</span>
              </label>
              <div className="flex items-center gap-1 mt-1">
                <input type="number" required value={form.order_qty}
                  onChange={(e) => setForm({ ...form, order_qty: e.target.value })}
                  className="flex-1 border rounded px-2 py-1.5" placeholder="수량" />
                <span className="text-sm text-gray-500 whitespace-nowrap">{MATERIAL_INFO[form.material_name]?.unit}</span>
              </div>
              {form.order_qty && (
                <p className="text-xs text-blue-600 mt-1">
                  → 라벨 약 {form.material_name === '라벨원단'
                    ? (Number(form.order_qty) * 25).toLocaleString()
                    : (Number(form.order_qty) * 10000).toLocaleString()}장 생산 가능
                </p>
              )}
        ...
        <div className="border rounded p-4 bg-gray-50 space-y-4 text-sm">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">아래 양식을 다운받아 작성 후 업로드하세요.</p>
              <div className="mt-1 text-xs text-gray-400 space-y-0.5">
                <p>• 품목: <span className="font-medium text-gray-600">라벨원단</span> 또는 <span className="font-medium text-gray-600">잉크</span> 중 하나</p>
                <p>• 날짜: <span className="font-medium text-gray-600">YYYY-MM-DD</span> 형식</p>
                <p>• 발주처·비고는 선택사항</p>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {previewRows.length > 0 && (
              <span className="text-xs text-blue-600">{previewRows.length}행 인식됨</span>
            )}
          </div>
                    ...
                    {previewRows.map((r, i) => (
                      <tr key={i} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-1.5 border">
                          {Number(r.order_qty).toLocaleString()} {MATERIAL_INFO[r.material_name]?.unit}
                        </td>
          ...
          {uploadResult && (
            <div className={`rounded p-3 text-xs space-y-1 ${uploadResult.fail === 0 ? 'bg-green-50 border border-green-200 text-green-700' : 'bg-yellow-50 border border-yellow-200 text-yellow-800'}`}>
              ...
              {uploadResult.failDetails?.map((d, i) => (
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-gray-100 text-gray-600 text-left">
            ...
            {showCancel && (
              <>
                <th className="px-3 py-2 border">재고 도착</th>
        ...
        <tbody>
          {rows.length === 0 && (
            <tr><td colSpan={colSpan} className="px-4 py-4 text-center text-gray-400">내역 없음</td></tr>
          )}
          {rows.map((o) => (
            <tr key={o.id} className="hover:bg-gray-50">
              <td className="px-3 py-2 border font-medium">{o.material_name}</td>
              <td className="px-3 py-2 border">{Number(o.order_qty).toLocaleString()}</td>
              ...
              {showCancel && (
                <>
                  <td className="px-3 py-2 border">
                    <button
                      onClick={() => onReceive(o.id, o.material_name, o.order_qty)}
