...
import { analyzeOrder, validateLabelCode } from './services/api'
      ...
      <div className="space-y-2 text-sm">
        ...
        <div>
          ...
          <div className="flex gap-1 mt-1">
            <input type="text" maxLength={9} value={form.label_code}
              onChange={(e) => { setForm({ ...form, label_code: e.target.value.toUpperCase() }); setValidation(null); setResult(null) }}
        ...
        <button onClick={handleAnalyze} disabled={loading || !form.label_code || !form.release_qty || !form.due_date}
        ...
        <div className={`mt-3 rounded border p-3 text-xs space-y-1 ${STATUS_CLS[result.deadline_status] || 'bg-gray-50 border-gray-200'}`}>
          ...
          {result.is_valid && (
            <>
              ...
              <p>필요 원단: {result.required_fabric_m}m · 잉크: {result.required_ink_count}통</p>
          ...
          <button key={i} onClick={() => select(d)} disabled={!d}
            className={`text-xs py-1.5 rounded text-center transition-colors
              ${!d ? 'invisible' :
                ...
                isToday(d)     ? 'bg-blue-50 text-blue-600 font-semibold ring-1 ring-blue-300' :
