"""
물류Agent 런처 — CMD 창 없이 백엔드 + 프론트 실행
더블클릭 또는 LogisticsAgent.exe 로 실행.

구조:
  backend/  — FastAPI uvicorn (127.0.0.1:8004)
  frontend/dist/ — 빌드된 정적 파일 (python -m http.server 3001)
  logs/     — 런타임 로그 저장

DEMO_MODE=1 인 경우 backend/.env 기준으로 기사/차량 시연 데이터 자동 seed.
"""
import datetime
import os
import subprocess
import sys
import time
import urllib.request
import webbrowser
from pathlib import Path

CREATE_NO_WINDOW = 0x08000000  # Windows: CMD 창 숨김

if getattr(sys, "frozen", False):
    # PyInstaller 번들 실행 시
    BASE = Path(sys.executable).parent
else:
    BASE = Path(__file__).resolve().parent

BACKEND_DIR = BASE / "backend"
FRONTEND_DIST = BASE / "frontend" / "dist"
FRONTEND_DIR = BASE / "frontend"
LOGS_DIR = BASE / "logs"
LOGS_DIR.mkdir(exist_ok=True)


def _ts() -> str:
    return datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")


def _log(msg: str) -> None:
    line = f"[{_ts()}] {msg}\n"
    try:
        with open(LOGS_DIR / "launcher.log", "a", encoding="utf-8") as f:
            f.write(line)
    except Exception:
        pass


def _find_python() -> str:
    for cmd in ["py", "python", "python3"]:
        try:
            r = subprocess.run([cmd, "--version"], capture_output=True, timeout=3)
            if r.returncode == 0:
                return cmd
        except Exception:
            pass
    return "python"


def _start_backend(py: str) -> tuple[subprocess.Popen, object]:
    log_file = open(LOGS_DIR / "backend.log", "a", encoding="utf-8", buffering=1)
    proc = subprocess.Popen(
        [py, "-m", "uvicorn", "main:app",
         "--host", "127.0.0.1",
         "--port", "8004"],
        cwd=str(BACKEND_DIR),
        stdout=log_file,
        stderr=log_file,
        creationflags=CREATE_NO_WINDOW,
    )
    return proc, log_file


def _start_frontend(py: str) -> tuple[subprocess.Popen, object]:
    log_file = open(LOGS_DIR / "frontend.log", "a", encoding="utf-8", buffering=1)

    if FRONTEND_DIST.exists():
        _log(f"프론트: dist 정적 서빙 — {FRONTEND_DIST}")
        proc = subprocess.Popen(
            [py, "-m", "http.server", "3001",
             "--directory", str(FRONTEND_DIST)],
            stdout=log_file,
            stderr=log_file,
            creationflags=CREATE_NO_WINDOW,
        )
    else:
        _log("프론트: dist 없음 — npm run dev 대체 사용")
        proc = subprocess.Popen(
            "npm run dev",
            cwd=str(FRONTEND_DIR),
            stdout=log_file,
            stderr=log_file,
            shell=True,
            creationflags=CREATE_NO_WINDOW,
        )
    return proc, log_file


def _wait_backend(timeout: int = 30) -> bool:
    for _ in range(timeout):
        try:
            urllib.request.urlopen("http://127.0.0.1:8004/", timeout=1)
            return True
        except Exception:
            time.sleep(1)
    return False


def main() -> None:
    _log("=" * 50)
    _log("물류Agent 런처 시작")
    _log(f"BASE={BASE}")

    py = _find_python()
    _log(f"Python: {py}")

    # ── 백엔드 시작 ──────────────────────────────────────────
    back_proc, back_log = _start_backend(py)
    _log(f"백엔드 PID={back_proc.pid} (포트 8004)")

    # ── 프론트엔드 시작 ──────────────────────────────────────
    front_proc, front_log = _start_frontend(py)
    _log(f"프론트 PID={front_proc.pid} (포트 3001)")

    # ── 백엔드 준비 대기 후 브라우저 열기 ────────────────────
    _log("백엔드 준비 대기 중...")
    ready = _wait_backend(30)
    if ready:
        _log("백엔드 준비 완료")
    else:
        _log("경고: 준비 대기 시간 초과 — 브라우저 열기 강행")

    webbrowser.open("http://localhost:3001")
    _log("브라우저 열기 완료 → http://localhost:3001")

    # ── 백엔드 종료 시까지 대기 ──────────────────────────────
    try:
        back_proc.wait()
    except KeyboardInterrupt:
        _log("KeyboardInterrupt — 종료 처리")
    finally:
        _log("프로세스 종료 중...")
        for proc in (back_proc, front_proc):
            try:
                proc.terminate()
            except Exception:
                pass
        for f in (back_log, front_log):
            try:
                f.close()
            except Exception:
                pass
        _log("물류Agent 런처 종료")


if __name__ == "__main__":
    main()
